...
function M.MoveItemBeforeMove(User, SourceItem, TargetItem)
	...
	if ( (ZielItem.id==2207) and (ZielItem.data==666) ) then
		...
		elseif ( (TargetItem.number>= 5) and (TargetItem.number<10) ) then
		    User:inform("moep->Wunschattribut +1 f�r 1 Stunde");
		elseif ( (TargetItem.number>=10) and (TargetItem.number<20) ) then
		    if ( EffektTyp<=5 ) then
  			User:inform("moep->Wunschattribut +2 f�r 1 Stunde");
		    else
			User:inform("moep->Wunschattribut +1 f�r 1 Stunden");
            		User:inform("moep->Randomattribut +1 f�r 1 Stunden");
            	    end
		elseif ( (TargetItem.number>=20) and (TargetItem.number<30) ) then
		    if ( EffektTyp<=5 ) then
			User:inform("moep->Wunschattribut +2 f�r 2 Stunden");
		    else
			User:inform("moep->Wunschattribut +2 f�r 1 Stunde");
			User:inform("moep->Randomattribut +2 f�r 1 Stunden");
		    end
		elseif ( (TargetItem.number>=30) and (TargetItem.number<40) ) then
		    if (EffektTyp<=3) then
			User:inform("moep->Wunschattribut +3 f�r 1 Stunde");
		    elseif ( (EffektTyp>3) and (EffektTyp<=6) ) then
			User:inform("moep->Wunschattribut +2 f�r 3 Stunden");
		    else
		        User:inform("moep->Wunschattribut +2 f�r 2 Stunden");
                	User:inform("moep->Randomattribut +2 f�r 1 Stunde");
		    end
		elseif ( (TargetItem.number>=40) and (TargetItem.number<50) ) then
		    if ( EffektTyp<=3 ) then
		        User:inform("moep->Wunschattribut +3 f�r 2 Stunden");
		    elseif ( (EffektTyp>3) and (EffektTyp<=6) ) then
			User:inform("moep->Wunschattribut +2 f�r 4 Stunden");
	 	    elseif ( (EffektTyp>6) and (EffektTyp<=8) ) then
			User:inform("moep->Wunschattribut +3 f�r 1 Stunde");
			User:inform("moep->Randomattribut +2 f�r 1 Stunden");
		    else
			User:inform("moep->Wunschattribut +2 f�r 2 Stunden");
		        User:inform("moep->Randomattribut +2 f�r 2 Stunden");
		    end
 		elseif ( (TargetItem.number>=50) and (TargetItem.number<60) ) then
		    if ( EffektTyp<=2 ) then
		        User:inform("moep->Wunschattribut +3 f�r 3 Stunden");
		    elseif ( (EffektTyp>2) and (EffektTyp<=4) ) then
                	User:inform("moep->Wunschattribut +2 f�r 5 Stunden");
		...
		elseif ( (TargetItem.number>=60) and (TargetItem.number<70) ) then
		    if ( EffektTyp<=2 ) then
		        User:inform("moep->Wunschattribut +4 f�r 1 Stunde");
		    elseif ( (EffektTyp>2) and (EffektTyp<=4) ) then
		        User:inform("moep->Wunschattribut +3 f�r 4 Stunden");
		    elseif ( (EffektTyp>4) and (EffektTyp<=6) ) then
		        User:inform("moep->Wunschattribut +2 f�r 6 Stunden");
		    elseif ( (EffektTyp>6) and (EffektTyp<=8) ) then
		        User:inform("moep->Wunschattribut +3 f�r 1 Stunde");
		        User:inform("moep->Randomattribut +3 f�r 1 Stunde");
		    else
		        User:inform("moep->Wunschattribut +3 f�r 2 Stunden");
		        User:inform("moep->Randomattribut +2 f�r 2 Stunden");
		    end
		elseif ( (TargetItem.number>=70) and (TargetItem.number<80) ) then
		    if ( EffektTyp<=2 ) then
		        User:inform("moep->Wunschattribut +4 f�r 2 Stunden");
		    elseif ( (EffektTyp>2) and (EffektTyp<=4) ) then
		        User:inform("moep->Wunschattribut +3 f�r 5 Stunden");
		    elseif ( (EffektTyp>4) and (EffektTyp<=6) ) then
		        User:inform("moep->Wunschattribut +2 f�r 7 Stunden");
		    elseif ( (EffektTyp>6) and (EffektTyp<=8) ) then
                	User:inform("moep->Wunschattribut +3 f�r 2 Stunden");
		        User:inform("moep->Randomattribut +3 f�r 1 Stunde");
		    else
		...
		elseif ( (TargetItem.number>=80) and (TargetItem.number<90) ) then
		    if ( EffektTyp<=2 ) then
		        User:inform("moep->Wunschattribut +4 f�r 3 Stunden");
		    elseif ( (EffektTyp>2) and (EffektTyp<=4) ) then
		        User:inform("moep->Wunschattribut +3 f�r 6 Stunden");
		    elseif ( (EffektTyp>4) and (EffektTyp<=6) ) then
		        User:inform("moep->Wunschattribut +2 f�r 8 Stunden");
		    elseif ( (EffektTyp>6) and (EffektTyp<=8) ) then
		        User:inform("moep->Wunschattribut +3 f�r 2 Stunde");
		        User:inform("moep->Randomattribut +3 f�r 2 Stunden");
		    else
		        User:inform("moep->Wunschattribut +4 f�r 1 Stunden");
		        User:inform("moep->Randomattribut +2 f�r 2 Stunden");
		    end
		elseif ( (TargetItem.number>=90) and (TargetItem.number<100) ) then
		    EffektTyp=math.random(12);
		    if ( EffektTyp<=2 ) then
		        User:inform("moep->Wunschattribut +5 f�r 1 Stunde");
			elseif ( (EffektTyp>2) and (EffektTyp<=4) ) then
			    User:inform("moep->Wunschattribut +4 f�r 4 Stunden");
			elseif ( (EffektTyp>4) and (EffektTyp<=6) ) then
			    User:inform("moep->Wunschattribut +3 f�r 7 Stunden");
			elseif ( (EffektTyp>6) and (EffektTyp<=8) ) then
			    User:inform("moep->Wunschattribut +2 f�r 9 Stunden");
		    ...
		    EffektTyp=math.random(12);
		    if ( EffektTyp<=2 ) then
		        User:inform("moep->Wunschattribut +5 f�r 2 Stunden");
			elseif ( (EffektTyp>2) and (EffektTyp<=4) ) then
			    User:inform("moep->Wunschattribut +4 f�r 5 Stunden");
			elseif ( (EffektTyp>4) and (EffektTyp<=6) ) then
			    User:inform("moep->Wunschattribut +3 f�r 8 Stunden");
			elseif ( (EffektTyp>6) and (EffektTyp<=8) ) then
			    User:inform("moep->Wunschattribut +2 f�r 10 Stunden");
			elseif ( (EffektTyp>8) and (EffektTyp<=10) ) then
			    User:inform("moep->Wunschattribut +4 f�r 1 Stunde");
			    User:inform("moep->Randomattribut +3 f�r 2 Stunden");
			else
...
function M.findText ()
	...
	elseif (number==6) then
	    deutsch="Denk noch einmal dar�ber nach";
	    englisch="Think about.";
	elseif (number==7) then
	    deutsch="Triff deine Entscheidung bald, sonst wird sie f�r dich getroffen.";
	    englisch="";
	else
