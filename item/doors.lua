...
function M.UseItem(User,SourceItem,TargetItem,counter,param)
    if base.doors.CloseDoor(SourceItem) then
        base.common.InformNLS(User,"Du schlie�t die T�r","You close the door.");
    else
        local OpenDoor,OpenOK=base.doors.OpenDoor(SourceItem);
        if OpenOK then
            base.common.InformNLS(User,"Du �ffnest die T�r.","You open the door.");
        elseif OpenDoor then
            base.common.InformNLS(User,"Du versuchst die T�r zu �ffnen, doch sie ist verschlossen.","You try to open the door, but the door is locked.");
        end
    end
end
