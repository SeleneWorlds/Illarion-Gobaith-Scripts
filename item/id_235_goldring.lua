...
function M.LookAtItem(User,Item)
	...
	elseif ( Item.data == 103) then -- Dajma�l's Ring
        if (User:getPlayerLanguage()==0) then
            world:itemInform(User,Item,"Du siehst einen goldenen Ring mit der Gravur: Dajma�l und Nordwind")
        else
            world:itemInform(User,Item,"You see a golden ring with the gravure: Dajma�l and Northwind")
        end
    ...
    elseif ( Item.data == 10000) then  -- Ring der Akademie
        if (User:getPlayerLanguage()==0) then
            world:itemInform(User,Item,"Du siehst einen auff�lligen Ring.");
        else
            world:itemInform(User,Item,"You see a peculiar dagger.");
        end
    elseif (Item.data == 800) then -- Questitem f�r Noradur (Char l�st sich ig auf - teleport zum gm schloss)
        if ( (User:isAdmin()) or (User.id==923280786)) then -- nur f�r gm
            if (User:getPlayerLanguage()==0) then
    ...
    elseif (Item.data == 801) then -- Questitem f�r Noradur (Firebreathing)
        if (User:isAdmin()) then -- nur f�r gm
            if (User:getPlayerLanguage()==0) then
...
function M.MoveItemAfterMove( User, SourceItem, TargetItem )
    ...
    function UseItem(User,SourceItem,TargetItem,counter,param,ltstate)
        ...
        if ( SourceItem.data == 100 ) then
            if ( TargetItem.id == 12 or TargetItem.id == 119 or TargetItem.id == 120 or TargetItem.id == 268 or TargetItem.id == 269 or TargetItem.id == 304 or TargetItem.id == 305 or TargetItem.id == 313 or TargetItem.id == 359 ) then
        ...
        if (SourceItem.data == 800) then
            if ( (User:isAdmin())  or (User.id==923280786) ) then -- nur f�r gm!!
                if ( string.find( User.lastSpokenText, "[Aa][Ss][Tt][Rr][Aa][Ll]" ) ~= nil ) then
                    User:talkLanguage( CCharacter.say, CPlayer.german, "#me's Konturen scheinen f�r einen kurzen Augenblick zu flimmern, bevor sich seine Gestalt vollst�ndig aufl�st.");
