	...
	if (SourceItem.data == 666) then
		if (User:getSkill("ancient language") > 59) then
		    if (SourceItem.pos.z == -6) then
		        if (User:getPlayerLanguage()==0) then
					User:inform("In deinen H�nden, Suchender, h�lst du die erste Weisheit. Fest und best�ndig wie die Erde ist auch die Weisheit. Alles Leben entspringt ihr und alles Leben kehrt zu ihr zur�ck.");
					User:inform("Um die zweite Weisheit zu finden sucht sie beim alten Heiligtum Bragons und ihr werdet ihn finden am Fu�e seiner hellsten Flamme.");
				else
			...
			elseif (SourceItem.pos.z == 1) then
	            if (User:getPlayerLanguage()==0) then
	                User:inform("In deinen H�nden, Suchender, h�lst du die zweite Weisheit. Unerbittlich und reinigend wie das Feuer ist auch die Weisheit. Wahre Weisheit brennt sich in unser Herz und schenkt uns Licht an dunklen Orten.");
	                User:inform("Die dritte Weisheit findet ihr in Findaris Umarmung �ber der tanzenden Ebene wo die Winde singen und die Berge uralte Geschichten fl�stern.");
	            else
	        ...
	        elseif (SourceItem.pos.z == 3) then
	            if (User:getPlayerLanguage()==0) then
	                User:inform("In deinen H�nden, Suchender, h�lst du die dritte Weisheit. Wie wir die Luft zum Atmen brauchen, so ist auch die Weisheit ein unersetzliches Gut. Endlos singt sie uns ihr Lied.");
	        ...
	        elseif (SourceItem.pos.z == 0) then
	            if (User:getPlayerLanguage()==0) then
	                User:inform("In deinen H�nden, Suchender, h�lst du die vierte Wahrheit. Rein und klar wie auch das Wasser ist sie f�r jene, die es wagen hineinzublicken. Ruhe bringt sie dem aufgew�hlten Geist.");
	                User:inform("Die f�nfte Weiheit kann nur finden, wer sie in sich selbst findet. Wer geschickt die vier in Wort verbindet, dem wird sich der Weg zu wahrer Weisheit �ffnen.");
	            else
