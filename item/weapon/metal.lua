...
function M.LookAtItem(User,Item)
    ...
    elseif ((Item.id==27) and (Item.data == 10000)) then
        if (User:getPlayerLanguage() == 0) then
            world:itemInform(User,Item,"Du siehst einen auff�lligen Dolch");            
        else
    ...
    if ((SourceItem.id == 2701) and (SourceItem.data == 50)) then
        if (User:getSkill("dwarf language") > 70) then
            if (User:getPlayerLanguage() == 0) then
                ...
                User:inform("Auf dem Schwert steht soviel wie: \"Ich bin das Schwert des Kriegers der in der Gunst der Vorv�ter der Norodaj steht.\"");
            else
