...
function M.isBurnable( id , lower, upper )
    if lower > upper then
        return false;
    end;
    ...
    elseif burnList[margin] < id then
        return M.isBurnable( id, margin+1, upper );
    else
        return M.isBurnable( id, lower, margin-1 );
    end;
end

function M.isBurnableField( fldPos )
    local fld = world:getField(fldPos);
    local cnt = fld:countItems();
    local i;
    for i=0,cnt-1 do
        if M.isBurnable( (fld:getStackItem(i)).id, 1, burnLength ) then
            return true;
        end;
    end;
    return false;
...
function M.isFireOnField(fldPos)
...
function M.addEffect(fireEffect, Patient)         -- Infection starts
...
function M.FireInRain( posX, posY, posZ )
...
function M.callEffect(fireEffect, NPC)
    ...
    while (i <= oldLstNo) and (i ~= 0) do
        ...
        if M.isFireOnField( flamePos ) then
                ...
                for y=posY-burnRad,posY+burnRad do
                    for z=posZ-1,posZ+1 do
                        if (math.random(1,4)==2) then
                            if ( not( M.FireInRain( x, y, z ) ) ) then -- Edit by abcfantasy: If fire not under the rain
                                burnPos = position(x,y,z);
                                if (world:isItemOnField( burnPos ) and not M.isFireOnField( burnPos )) and M.isBurnableField( burnPos ) then
