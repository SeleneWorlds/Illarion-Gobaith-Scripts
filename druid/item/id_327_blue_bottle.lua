...
function M.checkMissile(ItemData, lower, upper)
    if not lower then
        lower = 0;
    end
    if not upper then
    ...
    elseif listWK[margin] < ItemData then
        return M.checkMissile( ItemData, margin+1, upper );
    else
        return M.checkMissile( ItemData, lower, margin-1 );
    end
end
