...
function M.enemyNear(Monster,Enemy)
    ...
    if (MonID==144) then
        return ( drop.CastMonMagic(Monster,Enemy,5,{4000,5000},{{44,5},{46,5},{51,5}},{},40,1) == true );
    else
        return false;
    end
end
...
function M.onDeath(Monster)
    ...
    elseif (MonID==145) then --Son of Yogsothoththul, Level: 9, Armourtype: -, Weapontype: wrestling
        ...
        if not done then done=drop.AddDropItem(46,1,1,(100*math.random(8,9)+math.random(88,99)),0,2); end --ruby
        if not done then done=drop.AddDropItem(197,1,1,(100*math.random(8,9)+math.random(88,99)),0,2); end --amethyst
        if not done then done=drop.AddDropItem(284,1,1,(100*math.random(8,9)+math.random(88,99)),1,2); end --magic bluestone

        --Category 3: Weapon

        local done=drop.AddDropItem(280,1,20,(100*math.random(8,9)+math.random(88,99)),0,3); --diamond ring
        if not done then done=drop.AddDropItem(278,1,10,(100*math.random(8,9)+math.random(88,99)),0,3); end --blackstone ring
        if not done then done=drop.AddDropItem(68,1,1,(100*math.random(8,9)+math.random(88,99)),0,3); end --ruby ring
        if not done then done=drop.AddDropItem(277,1,1,(100*math.random(8,9)+math.random(88,99)),0,3); end --amethyst ring
