...
function M.enemyNear(Monster,Enemy)
    ...
    if (MonID==45) then
        return ( drop.CastMonMagic(Monster,Enemy,6,{1500,2000},{{51,5}},{},40,1,{50,60}) or drop.CastHealing( Monster, 6, {2000,3500}, 8, {16, 13}, 40 ) );
...
function M.enemyOnSight(Monster,Enemy)
    ...
    elseif (MonID==45) then
        return ( drop.CastMonMagic(Monster,Enemy,5,{1500,2000},{{51,5}},{},40,1,{50,60}) or drop.CastHealing( Monster, 5, {2000,3500}, 8, {16, 13}, 40 ) );
    else
        return false
    end
end
...
function M.onDeath(Monster)
    ...
    local MonID=Monster:get_mon_type();
        ...
        if not done then done=drop.AddDropItem(2286,1,10,(100*math.random(4,5)+math.random(44,55)),0,1); end --flame helmet
    ...
    elseif (MonID==43) then --Orc Thief, Level: 4, Armourtype: light, Weapontype: puncture
        ...
        local done=drop.AddDropItem(283,1,20,(100*math.random(3,4)+math.random(33,44)),0,2); --blackstone
        if not done then done=drop.AddDropItem(82,1,10,(100*math.random(3,4)+math.random(33,44)),0,2); end --blackstone amulet
        if not done then done=drop.AddDropItem(278,1,1,(100*math.random(3,4)+math.random(33,44)),0,2); end --blackstone ring
        if not done then done=drop.AddDropItem(252,1,1,(100*math.random(3,4)+math.random(33,44)),0,2); end --raw blackstone
