        ...
        if (thisNPC:isInRange(Originator,FollowInRange)) then
			...
			or (string.find(Message,"[Mm]ove")~=nil) then
				...
				if thisNPC:isInRange(Originator,1) then
                    ...
                    thisNPC:talkLanguage( CCharacter.say, CPlayer.german, "Meister warten...ich nicht so schnell sein." );
                    thisNPC:talkLanguage( CCharacter.say, CPlayer.english,"Master wait...I'm not that fast." );
				end
			end
        end
    end
