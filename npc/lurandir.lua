                ...
                if (string.find(message,NPCTrig[3-lang][i]) ~= nil) then
                    if ( (i==6) and (LuaAnd( originator:getQuestProgress(1), 65 ) ~= 0) and (LuaAnd( originator:getQuestProgress(1), 65536 ) == 0) ) then
                        ...
                        if ( ( (litem.id == 2496) and (litem.data == 3) ) or ( (ritem.id == 2496) and (ritem.data == 3) ) ) then
                            ...
                            thisNPC:talkLanguage(CCharacter.say,CPlayer.german ,"Ihr habt also das gesegnete Wasser von Irundar gefunden, mein Freund? So lasst es mich denn sehen!");
                            thisNPC:talkLanguage(CCharacter.say,CPlayer.english,"So you have found the blessed water of Irundar, my friend. Please, let me see it!");
                        end;
                    else
            			thisNPC:talk(CCharacter.say,NPCText[3-lang][i]);
            			if (i==3 or i==4) then
