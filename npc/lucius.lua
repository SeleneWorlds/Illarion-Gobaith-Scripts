...
--Name:        Lucius van Sareth
--Race:        human
--Town:        Varshikar
--Function:    Tax Collector
--Position:    x=267 y=-303 z=0
--Facing:      West
...
function M.initializeNpc()
    ...
    functions.AddTraderTrigger("[Yy]ou.+[Tt]rader","No. I'm no trader. I'm the tax collector of Varshikar.");
    functions.AddTraderTrigger("[DdIi][uh]r*.+[Hh]ändler","Nein ich bin kein Händler. Ich bin der Steuerneintreiber von Varshikar.");
    ...
    if functions.BasicNPCChecks(originator,2) then
        if (functions.LangOK(originator,TradSpeakLang)==true) then
            ...
            if (Status==1) then -- Gefragt nach Steuern, darf sie aber nicht nehmen
                gText="Ja. Ich sammel die Steuern der Händler ein. Aber dir werd ich das Geld ganz sicher nicht geben.";
