...
function M.UseItem(User,SourceItem,TargetItem,Counter,Param,ltstate)
    ...
    elseif (TargetItem.id == 329 ) then
        if ( Skill > 50) then
            if (modecode==0) then
                common.InformNLS(User,"Du schreibst eine Kurze Notiz in das Buch die dem n�chsten Lernenden Helfen wird, den Einstieg in die Sprache zu finden",
...
function M.LookAtItem(User,Item)
    ...
    if (modecode == 2) then
        world:itemInform(User,Item,common.GetNLS(User,"Buch des Ephraim","Book of Ephraim"));
    else
        world:itemInform(User,Item,common.GetNLS(User,"Lehrbuch der "..GetLanguage(langcode,false),"textbook of the "..GetLanguage(langcode,true)));
    end
end
