...
function M.GetCow(message, originator)
        ...
        if (originator:getQuestProgress(8) == 0) then
            ...
            if M.CheckMoney(originator,GCoins,SCoins,CCoins) then
                ...
                if not posOfCow then
                    ...
                    thisNPC:talkLanguage( CCharacter.say, CPlayer.german, "Hier habt ihr den Esel. Bringt sie sicher wieder dann bekommt ihr die "..(Kaution/100).." Silberstücke Kaution wieder.");
                    thisNPC:talkLanguage( CCharacter.say, CPlayer.english, "There you have your mule. Bring her back safely then you get your "..(Kaution/100).." silvercoins surety back.");
                ...
                thisNPC:talkLanguage( CCharacter.say, CPlayer.german, "Du hast nicht genug Geld. Ein Esel kostet "..PreisProKuh.." Kupferstücke und "..(Kaution/100).." Silberstücke müsst ihr als Kaution hinterlegen." );
    ...
    or (string.find(message,"buy.+donkey"   )~=nil) then
        if (string.find(message,"[Ee]sel")~=nil) or (string.find(message,"mule")~=nil) then
            thisNPC:talkLanguage( CCharacter.say, CPlayer.german, "Ich verkaufe keine Esel. Ich verleihe sie höchstens." );
            thisNPC:talkLanguage( CCharacter.say, CPlayer.english, "I don't sell mules. I just borrow them." );
        else
            thisNPC:talkLanguage( CCharacter.say, CPlayer.german, "Ich verkaufe keine Lasttiere. Ich verleihe sie höchstens." );
            thisNPC:talkLanguage( CCharacter.say, CPlayer.english, "I don't sell pack animals. I just borrow them." );
        end
    end
    return false
end
                    ...
                    if find_owner then
                        if (value_owner == originator.id) then
                            if world:deleteNPC( npc.id ) then
                                thisNPC:talkLanguage( CCharacter.say, CPlayer.german, "Danke für den Esel." );
