...
function M.initializeNpc()
    ...
    -- small talk
    autonpcfunctions.AddTraderTrigger("hello","Hello my friend. If you don't know where we were or what to do next, just say 'help'.");
    ...
    autonpcfunctions.AddAdditionalText("Be greeted. If you don't know where we were or what to do next, just say 'help'.");
    autonpcfunctions.AddTraderTrigger("hallo","Hallo mein Freund. Wenn Ihr nicht wisst, wo wir waren oder was Ihr als nächstes tun sollt, sagt einfach 'Hilfe'.");
	...
	autonpcfunctions.AddCondition("qpg","<",20);
    autonpcfunctions.AddAdditionalText("Seid gegrüßt. Wenn Ihr nicht wisst, wo wir waren oder was Ihr als nächstes tun sollt, sagt einfach 'Hilfe'.");
	autonpcfunctions.AddTraderTrigger("hello","Hello my friend.");
    autonpcfunctions.AddAdditionalTrigger("greet");
    autonpcfunctions.AddAdditionalTrigger("hey");
    autonpcfunctions.AddAdditionalTrigger("hiho");
    autonpcfunctions.AddCondition("lang","english");
    autonpcfunctions.AddAdditionalText("Be greeted.");
    ...
    autonpcfunctions.AddTraderTrigger("how are you","I'm fine, thanks. And you?");
    autonpcfunctions.AddTraderTrigger("wie geht dir","Mir geht es gut, danke. Und Euch?");
    autonpcfunctions.AddAdditionalTrigger("wie geht euch");
    autonpcfunctions.AddTraderTrigger("who","I am "..thisNPC.name..". I help all people who are new to Illarion.");
    autonpcfunctions.AddTraderTrigger("wer","Ich bin "..thisNPC.name..". Ich helfe allen Leuten, die neu in Illarion sind.");
    autonpcfunctions.AddTraderTrigger("my name","Nice to meet you. My name is "..thisNPC.name..".");
    autonpcfunctions.AddAdditionalTrigger("I am");
    autonpcfunctions.AddAdditionalTrigger("I'm");
    autonpcfunctions.AddTraderTrigger("mein name","Schön Euch zu treffen. Mein Name ist "..thisNPC.name..".");
    autonpcfunctions.AddAdditionalTrigger("ich bin");
    autonpcfunctions.AddTraderTrigger("what sell","I am not a trader.");
    autonpcfunctions.AddAdditionalTrigger("what buy");
    autonpcfunctions.AddTraderTrigger("was kauf","Ich bin kein Händler.");
    ...
    autonpcfunctions.AddAdditionalText("Macht's gut.");
    autonpcfunctions.AddAdditionalText("Lebt wohl.");
    -- explanations
    autonpcfunctions.AddTraderTrigger("tasche","Ihr habt ja gar keine Tasche mehr! Ich gebe Euch eine. Ihr seht sie auf der rechten Seite in Eurem Inventar, links oben neben dem Kopf. Ein Rechtsklick auf sie öffnet das Kontextmenü, wählt dann 'öffnen', um die Tasche zu öffnen.");
    autonpcfunctions.AddCondition("item",97,"body","=",0);
    autonpcfunctions.AddConsequence("item",97,1,333,0);
    autonpcfunctions.AddTraderTrigger("bag","Oh you don't have a bag anymore! I give you one. You can see it on the right side in your inventory, top left next to the head. A right click on it opens a context menu, then choose 'open' to open the bag.");
    autonpcfunctions.AddCondition("item",97,"body","=",0);
    autonpcfunctions.AddConsequence("item",97,1,333,0);
    autonpcfunctions.AddTraderTrigger("tasche","Ihr seht die Tasche auf der rechten Seite, in Eurem Inventar links oben neben dem Kopf. Ein Rechtsklick auf sie öffnet das Kontextmenü, wählt dann 'öffnen', um die Tasche zu öffnen.");
    autonpcfunctions.AddTraderTrigger("bag","You can see the bag on the right side, in your inventory top left next to the head. A right click on it opens a context menu, then choose 'open' to open the bag.");
	-- help?
	autonpcfunctions.AddTraderTrigger(".+","Hey, was macht Ihr denn hier? Ihr seid nicht neu. Geht den Weg entlang zum Schloss und nehmt die Portale am Ende der Halle. Sie führen nach Gobaith.");
	autonpcfunctions.AddCondition("lang","german");
	autonpcfunctions.AddCondition("qpg","<",4);
	autonpcfunctions.AddTraderTrigger(".+","Hey, what are you doing here? You are not new. Follow this way to the castle and take the portals at the other end of the hall. They lead to Gobaith.");
	autonpcfunctions.AddCondition("lang","english");
	autonpcfunctions.AddCondition("qpg","<",4);
	autonpcfunctions.AddTraderTrigger(".+","Tut mir Leid, ich habe Euch nicht verstanden. Sagt einfach 'Hilfe', wenn Ihr nicht wisst, was zu tun ist oder wie Ihr etwas tun sollt.");
	autonpcfunctions.AddCondition("lang","german");
	autonpcfunctions.AddTraderTrigger(".+","I am sorry, I didn't understand you. Say 'help' if you don't know what to do or how to do it.");
	autonpcfunctions.AddCondition("lang","english");
    -- ********* END DYNAMIC PART ********
...
function M.receiveText(texttype, message, originator)
	if (autonpcfunctions.BasicNPCChecks(originator,2)) then
		...
		elseif (Progress==11 or Progress==15) then
			if (string.find(message,"[Jj]a")~=nil or string.find(message,"[Yy]es")~=nil or revise) then
				if (originator:countItem(97)>0) then
					...
					outText=autonpcfunctions.GetNLS(originator,gText,eText);
					thisNPC:talk(CCharacter.say,outText);
					retVal=M.createBreadRoll(originator);
		...
		elseif Progress==13 then
			...
			string.find(message,"[Nn]ew.+[Bb]read.+[Rr]oll")~=nil or string.find(message,"[Aa]nother.+[Bb]read.+[Rr]oll")~=nil) then
				if M.createBreadRoll(originator) then
	...
	elseif M.HenryCheck(originator) then
		Progress=originator:getQuestProgress(2);
		thisNPC.activeLanguage=originator.activeLanguage;
		gText="nothing";
