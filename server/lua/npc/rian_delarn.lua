...
function M.initializeNpc()
    ...
    autonpcfunctions.functions.AddCycleText("#me beobachtet schweigend die Arena.","#me watches silently the arena.");
    autonpcfunctions.functions.AddCycleText("#me setzt sich auf den Baumstamm und beginnt in einem kleinen Buch zu lesen.","#me sit down at the log and starts to read in a small book.");
    autonpcfunctions.functions.AddCycleText("#me geht, leise Formeln vor sich hermurmelnd, langsam ein Stück auf und ab.","#me walks, mumbling silent formulas, a short way.");
...
function M.getCreature(message, originator)
	...
	GCoins,SCoins,CCoins = M.CalcSilverCopper(PreisHarmlKrea);
	...
	or (string.find(message,"aggressiv.+creatur.+summon")~=nil) then
        GCoins,SCoins,CCoins = M.CalcSilverCopper(PreisAggrKrea);
        if autonpcfunctions.CheckMoney(originator,GCoins,SCoins,CCoins) then
            M.Pay(originator,GCoins,SCoins,CCoins);
