...
function M.initializeNpc()
    ...
    functions.AddTraderTrigger("[Yy]ou.+[Tt]rader","No. I'm no trader. I'm the tax collector of Greenbriar.");
    functions.AddTraderTrigger("[DdIi][uh]r*.+[Hh]ändler","Nein ich bin kein Händler. Ich bin der Steuerneintreiber von Greenbriar.");
    ...
    functions.AddTraderTrigger("[Tt]ell.+[Ss]omething","Sorry, I'm pretty busy right now.");
    functions.AddTraderTrigger("[Ee]rzähl.+[Ww]as","Tut mir leid, ich bin grad ziemlich beschäftigt.");
    ...
    AllowedTaxCollectors[1737681585] = true; -- Miru Yukimitsu
    AllowedTaxCollectors[2010588785] = true; -- Lonsur
    ...
    if functions.BasicNPCChecks(originator,2) then
        if (functions.LangOK(originator,TradSpeakLang)==true) then
            ...
            if (Status==1) then -- Gefragt nach Steuern, darf sie aber nicht nehmen
                gText="Ja. Ich sammel die Steuern der Händler ein. Aber dir kann ich das Geld nicht geben.";
                eText="Indeed. I collect the money of the traders. But i can't give the money to you.";
...
function M.lookAtNpc(Char, mode)
    if initLook==nil then
        ...
        output[1]="A suspiciously glancing, small woman who has a stuffed purse attached to her belt. Sadly she is also holding a dangerlous-looking rolling pin.";
        initLook=1;
    end
