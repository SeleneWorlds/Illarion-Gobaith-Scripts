...
function M.loadBook(toData)
    ...
    base_books.AddEnglishBookText("The Cakebible",0,0,toData);
    base_books.AddEnglishBookText("Table of Contents:\nDedication Page 3\nPreparation Page 4-5\nRecipes Page 6-13\nSubclasses Page 14-17\nPhilosophies Page 18-19\nAcknowledgement Page 20-23",0,0,toData);
    base_books.AddEnglishBookText("Dedication:\n\nTo all who have a sweet tooth\nThe authors Lana von Maibach and Cassandria Littlethorn",0,0,toData);
    base_books.AddEnglishBookText("Preparation:\n\nBefore you get started on baking you surely need a lot of dough, a rolling pin to roll out the dough, a peel and of course the respective ingredients.",0,0,toData);
    base_books.AddEnglishBookText("After making the preparations let the fun begin.",0,0,toData);
    base_books.AddEnglishBookText("Breath of apple:\n\nTake two lumps of dough, put them on a surface covered with flour and knead them to a round form. Peel the apples and cut them into small pieces. Take one of the formed dough lumps",353,0,toData); -- ### Apple pie ###
    base_books.AddEnglishBookText("and distribute the apple slices on it more or further apart according to taste. Then put the remaining dough on top and bake it on moderate heat until the crust is golden brown. Let the cake cool down",0,0,toData);
    base_books.AddEnglishBookText("outdoor for half an hour and watch out that no halfling comes by to steal your cake. Then enjoy your meal!",0,0,toData);
    base_books.AddEnglishBookText("Cherry dream:\n\nFirst knead two lumps of dough to a round form. Then pit two times two handful of cherries. Distribute the cherries on one part of the dough and put the other part on top again. Bake",303,0,toData); -- ### Cherry cake ###
    base_books.AddEnglishBookText("it until the crust is light brown and then let the cake cool down for half an hour, but be careful that nobody steals it. Enjoy eating the cake with closed eyes!",0,0,toData);
    base_books.AddEnglishBookText("Symphony of sweet and sweeter:\nThis time knead three lumps of dough to a round form. Take the first part of the dough and distribute halved strawberries on it. Then repeat that with the second part",354,0,toData); -- ### Strawberry cake ###
    base_books.AddEnglishBookText("and cover the ensemble with the remaining dough. Then smooth the dough so that there are no gaps between the layers. Bake it on moderate heat until the crust is golden brown. Let the cake cool down",0,0,toData);
    base_books.AddEnglishBookText("for half an hour and after that put a few more strawberries on the edge. Close your eyes and enjoy the sweet symphony! Taste it!",0,0,toData);
    base_books.AddEnglishBookText("Blackberry cloud:\n\nTake two small dough lumps. Form one like a beaker and the other like a plate. Heat a honeycomb until it is completely fluid. Add a handfull ripe blackberries to the honey and",454,0,toData); -- ### Blackberry muffin ###
    base_books.AddEnglishBookText("pour it in the dough beaker. Put the dough plate on top and bake it about ten minutes. Now you can sleep on (blackberry) clouds!",0,0,toData);
    base_books.AddEnglishBookText("Sweet seduction:\n\nRoll out a piece of dough and cut out little cookies. Heat a honeycomb until it is completely fluid and brush the cookies with honey. After baking them in moderate heat for about",453,0,toData); -- ### Cookie ###
    base_books.AddEnglishBookText("fifteen minutes you may give in to temptation.",0,0,toData);
    base_books.AddEnglishBookText("Philosophies:\n\nThe quality of cakes (applicable to all kinds of sweets): The quality of cakes is measured against the bite marks in it. If there are so many bite marks that there isn't any cake",0,0,toData);
    base_books.AddEnglishBookText("left, you could say the cake was a good one.",0,0,toData);
    base_books.AddEnglishBookText("Acknowledgement: \n\nWe would like to thank all cakes, muffins and cookies which we eat up and which inspired us to write this bible. Furthermore we would like to thank the bakers without which all",0,0,toData);
    base_books.AddEnglishBookText("these treats wouldn't exist. Last but not least our friends who supplied us with sweets while we wrote this book.\n\n\nThe authors Lana von Maibach and Cassandria Littlethorn",0,0,toData);
    base_books.AddEnglishBookText("Have fun in Baking!",0,0,toData);
...
function loadTitle(toData)
    base_books.AddGermanBookTitle("Buch mit dem Titel \"Die Kuchenbibel\"",toData);
    base_books.AddEnglishBookTitle("Book with the title \"The Cake Bible\"",toData);
end;
