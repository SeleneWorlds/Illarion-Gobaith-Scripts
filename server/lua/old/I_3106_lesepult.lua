...
function M.UseItem(User,SourceItem,TargetItem,counter,param)
    ...
    if (User:getQuestProgress(100) == 0) then
        if (UserStat[User.id]~=SourceItem.data) then
            ...
            if (User:getPlayerLanguage()==0) then
                User:inform("Möchtest du wirklich für "..M.VotingTargets(SourceItem.data).." abstimmen? Dann benutze das Lesepult erneut");
            else
            ...
            if (User:getPlayerLanguage()==0) then
                User:inform("Du schreibst deinen Namen auf das Pergament und deine Schrift ist nur einen Moment sichtbar ehe sie verschwindet. Trotzdem bist du dir nun sicher für "..M.VotingTargets(SourceItem.data).." abgestimmt zu haben.");
            else
...
function M.LookAtItem(User,Item)
    if (M.VotingTargets(Item.data) ~= "") then
        if (User:getPlayerLanguage()==0) then
            world:itemInform(User,Item,"Du siehst ein fast leeres Buch. Nur eine Zeile steht dort: \"Schreibt hier euren Namen ein um für "..M.VotingTargets(Item.data).." zu stimmen.\"");
        else
