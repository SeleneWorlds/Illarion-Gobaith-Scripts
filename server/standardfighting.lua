...
--]]
    ...
    -- Check if the Attacked character has to be killed by a coup de gr�ce
    if CoupDeGrace( Attacker, Defender ) then
---[[DEBUG
        AddDebugText( "Attack ended due Coup de gr�ce", 3, Attacker.Char );
--]]
        return true;
    end
---[[DEBUG
    AddDebugText( "No Coup de gr�ce was done", 3, Attacker.Char );
--]]
...
function InitDebug( Char )
    if DebugLevel > 0 then
        Char:increaseSkill(1,"common language",100-Char:getSkill("common language"));
    end
...
--[[
    Checks if a coup de gr�ce is performed on the attacked character and kills the char if needed

    @param  CharacterTable  The table of the attacking character
    @param  CharacterTable  The table of the attacked character
    @return boolean  true if a coup de gr�ce was done
]]
function CoupDeGrace( Attacker, Defender )
    ...
    if (( Attacker.AttackKind ~= 4 ) and ( Hitpoints == 1 ) and not IsTrainingWeapon( Attacker.WeaponItem.id )) then
        ...
        Attacker.Char:talkLanguage( CCharacter.say, CPlayer.german,  "#me gibt "..gText.." Gegner den Gnadensto�." );
...
--[[
    ...
    Also here the Coup de GrÃ¢ce is done
